<template>
  <div class="h-screen flex items-center">
    <div class="w-full max-w-md mx-auto py-12 px-6 bg-white shadow rounded-md">
      ...
      <p class="my-5 text-base leading-5 text-center text-gray-900">
      ...
      <form @submit.prevent="onSubmit">
        <div class="rounded-md shadow-sm">
          ...
          <div class="mt-5">
            <button type="submit" class="button button-blue w-full rounded-md">
              Sign In
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async onSubmit() {
      const promise = appwrite.account.createSession(this.email, this.password);

      promise.then(
        () => {
          const newPromise = appwrite.account.get();
          newPromise.then(
            (response) => {
